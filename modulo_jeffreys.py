...
@jit(nopython = True)
def calc_ku(u: np.ndarray, d2Tdx2: np.ndarray, alfa: np.ndarray, tau_q: float, tau_gT: float, dx: float) -> np.ndarray:
    d2udx2 = d2dx2(u, dx, full = False)
    return (alfa * (d2Tdx2 + tau_gT * d2udx2) - u) / tau_q

@jit(nopython = True)
def rungekutta_u(u: np.ndarray, T: np.ndarray, alfa: np.ndarray, tau_q: float, tau_gT: float, dx: float, dt: float) -> np.ndarray:
    d2Tdx2 = d2dx2(T, dx, full = False)
    #Calculo de k1
    y1 = np.copy(u)
    k1 = calc_ku(y1, d2Tdx2, alfa, tau_q, tau_gT, dx)
    #Calculo de k2
    y2 = y1 + 0.5 * dt * k1
    k2 = calc_ku(y2, d2Tdx2, alfa, tau_q, tau_gT, dx)
    #Calculo de k3
    y3 = y1 + 0.5 * dt * k2
    k3 = calc_ku(y3, d2Tdx2, alfa, tau_q, tau_gT, dx)
    #Calculo de k4
    y4 = y1 + dt * k3
    k4 = calc_ku(y4, d2Tdx2, alfa, tau_q, tau_gT, dx)
    #Resultado
    return u + (dt / 6.0) * (k1 + 2.0 * (k2 + k3) + k4)

@jit(nopython = True)
def calc_kq(q: np.ndarray, dTdx: np.ndarray, dudx: np.ndarray, tau_q: float, tau_gT: float) -> np.ndarray:
    return - (dTdx + tau_gT * dudx + q) / tau_q

@jit(nopython = True)
def rungekutta_q(q: np.ndarray, T: np.ndarray, u: np.ndarray, tau_q: float, tau_gT: float, dx: float, dt: float) -> np.ndarray:
    dTdx = ddx(T, dx, full = False)
    dudx = ddx(u, dx, full = False)
    #Calculo de k1
    y1 = np.copy(q)
    k1 = calc_kq(y1, dTdx, dudx, tau_q, tau_gT)
    #Calculo de k2
    y2 = y1 + 0.5 * dt * k1
    k2 = calc_kq(y2, dTdx, dudx, tau_q, tau_gT)
    #Calculo de k3
    y3 = y1 + 0.5 * dt * k2
    k3 = calc_kq(y3, dTdx, dudx, tau_q, tau_gT)
    #Calculo de k4
    y4 = y1 + dt * k3
    k4 = calc_kq(y4, dTdx, dudx, tau_q, tau_gT)
    #Resultado
